...
for i in dictionnaireDemarches:
    ...
    fichier = open("./Data/" + nomDemarche + ".csv", "a")
    ...
    if elem == "Il n'existe plus de plage horaire libre pour votre demande de rendez-vous. Veuillez recommencer ultérieurement.":
        ...
        elem = driver.find_element_by_xpath("//table/tbody/tr/td")
        for i in elem:
    ...
    fichier.write("\n" + str(jour) + "," + str(heure) + "," + str(disponibilite))
    fichier.close()
    print("Il y a " + str(disponibilite) + " disponibilités\n")
